...
| `inlineThinkTagModels?` | `string[]` | `openai-chat` recovers inline reasoning by default when a reply begins with `<think>` / `<thinking>` / `<reasoning>` (optionally after whitespace). An explicit list limits recovery to those models; `[]` disables it. Streamed and buffered replies preserve answer whitespace. Subsequent tags delimit reasoning anywhere, including code fences. Ordinary text before the first tag keeps the reply untouched. Prefer structured upstream reasoning where supported. |
