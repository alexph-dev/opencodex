...
| 方法與路徑 | 用途 | 主要錯誤 |
| --- | --- | --- |
| `POST /api/client-integrations/preview` | 為單一用戶端規劃 `apply`、`overwrite` 或 `disable`；請求內容為 `{ "clientId": "...", "operation": "..." }` | 400 用戶端或操作無效；409 `integration_preview_unavailable` |
| `POST /api/client-integrations/restore/preview` | 規劃一次復原；請求內容為 `{ "opId": "...", "confirmDrift": false }` | 404 操作不存在；409 `integration_preview_unavailable` |
