...
| 方法与路径 | 用途 | 主要错误 |
| --- | --- | --- |
| `POST /api/client-integrations/preview` | 为一个客户端规划 `apply`、`overwrite` 或 `disable`；请求体为 `{ "clientId": "...", "operation": "..." }` | 400 客户端或操作无效；409 `integration_preview_unavailable` |
| `POST /api/client-integrations/restore/preview` | 规划一次撤销；请求体为 `{ "opId": "...", "confirmDrift": false }` | 404 操作不存在；409 `integration_preview_unavailable` |
