...
запишет, например повторное применение уже применённого.

`integration_preview_unavailable` означает, что список моделей ещё не сформирован: так выглядит
только что запущенный прокси. Одно чтение `GET /api/client-integrations` его формирует.
