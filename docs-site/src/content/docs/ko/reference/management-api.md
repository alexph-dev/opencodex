...
| 메서드 및 경로 | 목적 | 주요 오류 |
| --- | --- | --- |
| `POST /api/client-integrations/preview` | 클라이언트 하나의 `apply`, `overwrite`, `disable`을 계획합니다. 본문은 `{ "clientId": "...", "operation": "..." }` | 400 잘못된 클라이언트나 작업, 409 `integration_preview_unavailable` |
| `POST /api/client-integrations/restore/preview` | 실행 취소를 계획합니다. 본문은 `{ "opId": "...", "confirmDrift": false }` | 404 없는 작업, 409 `integration_preview_unavailable` |
