...
describe("freeform wrapper keys the literal matcher could not see", () => {
  ...
  test("a preceding value larger than the scan budget holds rather than guessing", () => {
    ...
    // bound the wrapper is still unwrapped, by the authoritative parse at completion: the
    // budget costs preview, never agreement.
    ...
    expect(emissions(body, "exec")).toEqual(["cmd"]);
  });
