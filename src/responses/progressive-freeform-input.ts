...
import { JSON_ESCAPES, JSON_WHITESPACE, scanFreeformWrapper } from "./freeform-wrapper-scan";
...
export function progressiveFreeformInput(args: string, toolName: string): string | null {
  ...
  // The scan reports whether parsing could possibly succeed. When it ran out of buffer the
  // object is provably incomplete and no parse is attempted at all, which is what keeps a
  // wrapper holding a large value from re-parsing a growing buffer on every delta. When the
  // scan ran out of its own budget it knows nothing, so fall back to the cheap test that a
  // complete object ends on its closing brace: that skips parses certain to fail without
  // changing any outcome, and it is all that remains of this cost.
  if (!scan.parse) return null;
  if (!closesAnObject(args)) return null;
  try {
    JSON.parse(args);
  } catch {
    return null;
...
/** Whether the buffer's last non-whitespace character closes an object. */
function closesAnObject(text: string): boolean {
  let i = text.length - 1;
  while (i >= 0 && JSON_WHITESPACE.has(text[i]!)) i--;
  return i >= 0 && text[i] === "}";
}
