...
export function scanFreeformWrapper(text: string): FreeformWrapperScan {
  ...
  const bounded = text.length > MAX_FREEFORM_WRAPPER_SCAN_CHARS
    ...
    : text;
  // A hold that ran out of BUFFER proves the object is not complete, so nothing is gained by
  // parsing it. A hold that ran out of BUDGET proves nothing, and the caller has to fall back
  // to testing whether the text could be a complete object. Reporting which one happened is
  // what keeps the common case from re-parsing a growing buffer on every delta.
  const hold = (): FreeformWrapperScan => ({ kind: "hold", parse: bounded.length < text.length });
